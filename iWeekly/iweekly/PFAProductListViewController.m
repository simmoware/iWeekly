//
//  PFAProductListViewController.m
//  iWeekly
//
...
@interface PFAProductListViewController ()

@end

@implementation PFAProductListViewController

@synthesize masterList, searchableList, searchBar, tableView, api;
...
- (void)viewDidLoad {
    ...
    [request start:@"/GetAllProductDescription" withParams:@{}];
...
- (BOOL) PFAAPIRequestDidComplete:(NSData *)response {
    ...
    sortedResults = [results sortedArrayUsingComparator:^NSComparisonResult(id a, id b) {
        NSString *first = [(NSDictionary*)a objectForKey:@"des"];
        NSString *second = [(NSDictionary*)b objectForKey:@"des"];
    ...
    for (NSDictionary *dict in sortedResults) {
        [masterList addObject:[[dict valueForKey:@"des"] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]]];
        [searchableList addObject:[[dict valueForKey:@"des"] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]]];
    }
...
- (void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    NSString *item = [searchableList objectAtIndex:indexPath.row];
    if (!self->forOrders) {
        PFAProductSupplierViewController *supp = [[PFAProductSupplierViewController alloc] initWithNibName:nil bundle:nil];
        [supp setProductIdentifier:item];
        ...
        item = [item stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLHostAllowedCharacterSet]];
        api = [[PFASupplierAPI alloc] initWithDelegate:self];
        [api act:[NSString stringWithFormat:@"/api/v1/itemsorders?itemDescription=%@", item] andFollowUp:nil];
    }
}
