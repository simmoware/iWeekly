...
@interface PFAProductBySupplierViewController ()

@end

@implementation PFAProductBySupplierViewController
...
- (void)viewDidLoad {
    ...
    if (![self.bannerName isEqualToString:@"ALL"]) {
        [request start:@"/GetPODetailsByDesAndBanner" withParams:@{@"des":self.productIdentifier, @"banner":self.bannerName}];
    } else {
        [request start:@"/GetPODetailsByDesAndAllBanners" withParams:@{@"des":self.productIdentifier}];
    }
    ...
    [self setTitle:[NSString stringWithFormat:@"%@ - %@", self.bannerName, self.productIdentifier]];
}
...
- (BOOL) PFAAPIRequestDidComplete:(NSData *)response {
    ...
    for (NSDictionary *dict in objects) {
        ...
        NSString *key = [NSString stringWithFormat:@"%@ - Σ%@", [df_out stringFromDate:date], [dict valueForKey:@"Total"]];
...
- (UIView *) tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    lbl = [[UILabel alloc] initWithFrame:CGRectMake(0, 34, self.view.frame.size.width*0.18, 30)];
    [lbl setTextAlignment:NSTextAlignmentCenter];
        [lbl setBackgroundColor:[UIColor colorWithWhite:0.0 alpha:0.0]];
    ...
    lbl = [[UILabel alloc] initWithFrame:CGRectMake(self.view.frame.size.width*0.18, 34, self.view.frame.size.width*0.20, 30)];
    [lbl setTextAlignment:NSTextAlignmentCenter];
    [lbl setBackgroundColor:[UIColor colorWithWhite:0.0 alpha:0.0]];
    [lbl setText:@"PO"];
    [lbl setFont:[UIFont fontWithName:@"Lato-Bold" size:headerFontSize]];
    ...
    lbl = [[UILabel alloc] initWithFrame:CGRectMake(self.view.frame.size.width*0.38, 34, self.view.frame.size.width*0.10, 30)];
    [lbl setTextAlignment:NSTextAlignmentCenter];
        [lbl setBackgroundColor:[UIColor colorWithWhite:0.0 alpha:0.0]];
    [lbl setText:@"QTY"];
    [lbl setFont:[UIFont fontWithName:@"Lato-Bold" size:headerFontSize]];
    ...
    lbl = [[UILabel alloc] initWithFrame:CGRectMake(self.view.frame.size.width*0.48, 34, self.view.frame.size.width*0.15, 30)];
    [lbl setTextAlignment:NSTextAlignmentCenter];
        [lbl setBackgroundColor:[UIColor colorWithWhite:0.0 alpha:0.0]];
    [lbl setText:@"Price"];
    [lbl setFont:[UIFont fontWithName:@"Lato-Bold" size:headerFontSize]];
    ...
    lbl = [[UILabel alloc] initWithFrame:CGRectMake(self.view.frame.size.width*0.63, 34, self.view.frame.size.width*0.10, 30)];
    [lbl setTextAlignment:NSTextAlignmentCenter];
        [lbl setBackgroundColor:[UIColor colorWithWhite:0.0 alpha:0.0]];
    [lbl setText:@"ORG Q"];
    [lbl setFont:[UIFont fontWithName:@"Lato-Bold" size:headerFontSize]];
    ...
    lbl = [[UILabel alloc] initWithFrame:CGRectMake(self.view.frame.size.width*0.73, 34, self.view.frame.size.width*0.15, 30)];
    [lbl setTextAlignment:NSTextAlignmentCenter];
        [lbl setBackgroundColor:[UIColor colorWithWhite:0.0 alpha:0.0]];
    [lbl setText:@"ORG $"];
    [lbl setFont:[UIFont fontWithName:@"Lato-Bold" size:headerFontSize]];
...
- (UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    for (UIView *v in subviews) {
        if ([v isKindOfClass:[UILabel class]] && ![v isEqual:cell.textLabel]) {
            ...
            if (count == 0) {
                [txt setText:[self prepareString:[item objectForKey:@"CUS"]]];
                ...
                [txt setFrame:CGRectMake(self.view.frame.size.width*0.18, 0, self.view.frame.size.width*0.2, rowHeight)];
            } else if (count == 2) {
                [txt setText:[self prepareString:[item objectForKey:@"qty"]]];
                [txt setFrame:CGRectMake(self.view.frame.size.width*0.38, 0, self.view.frame.size.width*0.10, rowHeight)];
            } else if (count == 3) {
                [txt setText:[self prepareString:[formatter stringFromNumber:[NSNumber numberWithFloat:[[item objectForKey:@"price"] floatValue]]]]];
                [txt setFrame:CGRectMake(self.view.frame.size.width*0.48, 0, self.view.frame.size.width*0.15, rowHeight)];
            } else if (count == 4) {
                [txt setText:[self prepareString:[item objectForKey:@"original_order_qty"]]];
                [txt setFrame:CGRectMake(self.view.frame.size.width*0.63, 0, self.view.frame.size.width*0.10, rowHeight)];
            } else if (count == 5) {
                ...
                [txt setFrame:CGRectMake(self.view.frame.size.width*0.73, 0, self.view.frame.size.width*0.15, rowHeight)];
            } else if (count == 6) {
...
- (void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    NSDictionary *item = [[keys objectForKey:[results objectAtIndex:indexPath.section]] objectAtIndex:indexPath.row];
    PFAPOProductDetailViewController *prod = [[PFAPOProductDetailViewController alloc] initWithStyle:UITableViewStylePlain];
    [prod setProductIdentifier:self.productIdentifier];
    [prod setCusCode:[self prepareString:[item valueForKey:@"CUS"]]];
